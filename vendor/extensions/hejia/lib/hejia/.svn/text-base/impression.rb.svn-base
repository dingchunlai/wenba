...
# Currently used add Coupon::Distributor 
#
# Anthor: U
# Time: 2010-01-21
#
  ...
  module Impression # nodoc...
    ...
    module InstanceMethods
      ...
      def impression_validates( impression_title, impression_number )
        impression_errors.add_to_base('请填写印象' ) if impression_title.blank?
        impression_errors.add_to_base('印象不能超过5个字') if impression_title.mb_chars.size > 5
        impression_errors.add_to_base('得票数必须是个整数') unless impression_number.is_a?(Integer)
        impression_errors.add_to_base('得票数不能少于1') if impression_number.is_a?(Integer) && impression_number < 1
      end
      ...
      def validate_impression_count(key, expired_at)
        if Hejia[:cache].get(key).nil?
          ...
          impression_errors.add_to_base('对不起，每天最多可投两个印象')
        end
      end
